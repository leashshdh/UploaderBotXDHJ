...
async def get_link(bot, update):
    ...
    if update.reply_to_message is not None:
        ...
        command_to_exec = ["curl", "https://api.gofile.io/getServer"]
        try:
        ...
        end_one = datetime.now()
        command_to_exec = ["curl", "-F", f'file=@"{after_download_file_name}"', url]
        await bot.edit_message_text(
        ...
        await bot.edit_message_text(
            chat_id=update.chat.id,
            text=Translation.AFTER_GET_GOFILE_LINK.format(
                t_response_ray[29],
                humanbytes(s0ze),
                t_response_ray[33],
                t_response_ray[13],
